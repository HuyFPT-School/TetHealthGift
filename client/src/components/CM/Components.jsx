...
export const Spinner = ({ size = 20 }) => (
  <span
    className="spinner"
    style={{ width: size, height: size, border: `2px solid #eee`, borderTopColor: "var(--red)" }}
  />
);
...
export const ORDER_STATUS = {
  processing: { label: "Chờ xác nhận", color: "#92400E", bg: "#FEF3C7", next: "shipped"   },
  shipped:    { label: "Đang giao",     color: "#1E40AF", bg: "#DBEAFE", next: "delivered" },
  delivered:  { label: "Hoàn thành",   color: "#065F46", bg: "#D1FAE5", next: null         },
  cancelled:  { label: "Đã huỷ",       color: "#991B1B", bg: "#FEE2E2", next: null         },
};

export const StatusBadge = ({ status }) => {
  const c = ORDER_STATUS[status] || { label: status, color: "#555", bg: "#F5F5F5" };
  return (
