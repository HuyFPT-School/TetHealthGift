...
const fieldClassName =
  "w-full h-[44px] px-3 rounded-[10px] border-[1.5px] border-[#f0d0a0] bg-white text-[14px] outline-none transition-[border-color,box-shadow] duration-200 focus:border-[#F9A825] focus:shadow-[0_0_0_3px_rgba(249,168,37,0.15)]";
...
export default function ProfileSection({
  ...
  const renderDropdown = (
    ...
    <div className="relative">
      ...
      {openDropdown === type && (
        <div className="absolute z-10 mt-2 w-full bg-white border border-[#f0d0a0] rounded-[10px] shadow-[0_12px_30px_rgba(41,10,24,0.08)]">
            ...
            {options.length ? (
              options.map((option) => (
                <button
                  ...
                  className="w-full text-left px-3 py-2 text-[14px] text-[#2b2730] hover:bg-[#fff4e6]"
                >
                  {option.name}
                </button>
              ))
  ...
  return (
    <div ref={containerRef} className="flex flex-col gap-6">
      ...
      <form className="flex flex-col gap-4" onSubmit={onSubmit}>
        ...
        {/* Address Section */}
        <div className="border-t border-[#f0d0a0] pt-4 mt-2">
          ...
          {!isAddressFormOpen && !profileAddress && !hasAddress && (
            <button
              type="button"
              onClick={() => setIsAddressFormOpen(true)}
              className="h-[40px] px-4 rounded-[10px] border-0 bg-[#C62828] text-white text-[13px] font-semibold shadow-[0_8px_16px_rgba(198,40,40,0.35)]"
            >
              + Thêm địa chỉ
            </button>
          )}
          ...
          {!isAddressFormOpen && (profileAddress || hasAddress) && (
            <div className="px-4 py-3 rounded-[10px] bg-[#fff4e6] text-[14px] text-[#2b2730]">
        ...
        <div>
          <button
            type="submit"
            disabled={loading}
            className="h-[44px] px-6 rounded-[12px] border-0 bg-[#C62828] text-white font-semibold text-[15px] cursor-pointer shadow-[0_8px_16px_rgba(198,40,40,0.35)] disabled:opacity-70 disabled:cursor-not-allowed"
          >
